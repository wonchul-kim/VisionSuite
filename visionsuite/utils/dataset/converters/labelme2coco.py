...
def main():
    ...
    #     sys.exit(1)
    os.makedirs(osp.join(args.output_dir, args.mode), exist_ok=True)
    if not args.noviz:
        os.makedirs(osp.join(args.output_dir, args.mode, "Visualization"), exist_ok=True)
    ...
    class_name_to_id = {}
    out_ann_file = osp.join(args.output_dir, f"{args.mode}.json")
    label_files = glob.glob(osp.join(args.input_dir, args.mode, "*.json"))
    for image_id, filename in tqdm(enumerate(label_files)):
        ...
        img_file = osp.join(args.input_dir, args.mode, base + image_ext)
        ...
        if not args.noviz:
            ...
            out_viz_file = osp.join(args.output_dir, args.mode, "Visualization", base + image_ext)
