...
def process_images(image_directory, sam_model_name, threshold, batch_size):
    ...
    # SAM2 모델 로드
    sam_model = sam_model_registry['vit_l'](checkpoint="/HDD/weights/sam2/sam2_hiera_large.pth").to(device)
    sam_model.eval()

    # SAM2 이미지 변환 (224x224이 아니라 1024x1024 사용)
    ...
    damaged_image_ids, all_embeddings = generate_sam2_embeddings(
        all_image_ids, images_to_paths, sam_model, transform, device, batch_size, regenerate_embeddings, embeddings_file
    )
...
# SAM2 임베딩 추출 함수
def generate_sam2_embeddings(image_ids, image_paths, model, transform, device, batch_size, regenerate, embeddings_file):
    ...
    with torch.no_grad():
        for i in range(0, len(image_ids), batch_size):
            ...
            if batch_images:
                ...
                features = model.image_encoder(batch_images)  # SAM2 이미지 임베딩
                features = features.mean(dim=(2, 3)).cpu().numpy()  # Spatial 평균 풀링
