...
embedding_dir = '/HDD/datasets/projects/benchmarks/mr_ad_bench/embedding_data_each'
cluster_dir = '/HDD/datasets/projects/benchmarks/mr_ad_bench/clustered/dinov2-large/attention_False'
output_dir = '/HDD/datasets/projects/benchmarks/mr_ad_bench/clustered/dinov2-large/attention_False'

search_all = True
if search_all:
    folders = [folder.split("/")[-1] for folder in glob(osp.join(embedding_dir, "**")) if not osp.isfile(folder)]
    embeddings = [] 
    filenames = []
    for folder in tqdm(folders):
        embedding_files = sorted(glob(osp.join(embedding_dir, folder, 'dinov2-large/attention_False', '*.npy')))
        filename_files = sorted(glob(osp.join(embedding_dir, folder, 'dinov2-large/attention_False', '*.txt')))
        for embedding_file, filename_file in zip(embedding_files, filename_files):
            embeddings.append(np.load(embedding_file, mmap_mode='r'))
            ori_index_f = open(osp.join(filename_file), 'r')
            while True:
    ...
    ori_data = np.concatenate(embeddings, axis=0)
    ori_index = filenames
else:
...
print("index length: ", len(ori_index))

assert ori_data.shape[0] == len(ori_index)
fig, ax = plt.subplots(1, 1, figsize=(6 * 4, 5))
ax.scatter(ori_data[:, 0], ori_data[:, 1], alpha=0.2, color='r')

data1 = np.load(osp.join(cluster_dir, 'level1/centroids.npy'), mmap_mode="r")
data2 = np.load(osp.join(cluster_dir, 'level2/centroids.npy'), mmap_mode="r")
data3 = np.load(osp.join(cluster_dir, 'level3/centroids.npy'), mmap_mode="r")
data4 = np.load(osp.join(cluster_dir, 'level4/centroids.npy'), mmap_mode="r")
data5 = np.load(osp.join(cluster_dir, 'level5/centroids.npy'), mmap_mode="r")
data6 = np.load(osp.join(cluster_dir, 'level6/centroids.npy'), mmap_mode="r")
data7 = np.load(osp.join(cluster_dir, 'level7/centroids.npy'), mmap_mode="r")

clusters = {}
counts = []
for _level in range(1, last_level + 1):
    sorted_clusters = np.load(osp.join(cluster_dir, f'level{_level}/sorted_clusters.npy'), allow_pickle=True)
...
if make_clustered_dataset:
    for level in levels:
        ...
        sorted_clusters = np.load(osp.join(cluster_dir, f'level{level}/sorted_clusters.npy'), allow_pickle=True)
        ...
        #         img_file = osp.join(input_dir, folder, ori_index[index] + '.bmp')
        #         json_file = osp.join(input_dir, folder, ori_index[index] + '.json')
        #         if osp.exists(img_file) and osp.exists(json_file):
        #             cnt += 1
        ...
        #     assert cnt == 1, ValueError(f"There is no such image file: {img_file}")
        ...
        for jdx, filename in tqdm(enumerate(ori_index)):
            if jdx not in sorted_clusters:
                cnt = 0
                for folder in folders:
                    img_file = osp.join(input_dir, folder, ori_index[jdx] + '.bmp')
                    json_file = osp.join(input_dir, folder, ori_index[jdx] + '.json')
                    if osp.exists(img_file) and osp.exists(json_file):
                        cnt += 1
                    # assert osp.exists(img_file), RuntimeError(f'There is no such image file: {img_file}')
                    # assert osp.exists(json_file), RuntimeError(f'There is no such json file: {json_file}')
