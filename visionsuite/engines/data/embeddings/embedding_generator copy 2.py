    ...
    def set_model(self):
        ...
        if 'dinov2' in config['model_name']:
            ...
            self._model = Dinov2FromHuggingFace(self._config['output_dir'], self._config['model_name'],
                                                output_attentions=True if self._config['post']['type'] in ['attention', 'mot'] else False)
            ...
            for x, filename in tqdm(self._dataloader):
                ...
                if 'post' in self._config and self._config['post']:
                    if self._config['post']['type'] == 'clustering':
                        ...
                        cls_attn = attn[:, :, 0, 1:]         # cls_attn: (B, heads, tokens) - cls token of patch
                        avg_attn = cls_attn.mean(dim=1)      # avg_attn: (B, tokens)
                    ...
                    elif self._config['post']['type'] == 'most':
                        ### 3. self-attention 
                        ...
                        A = (feats @ feats.transpose(1, 2)).squeeze() # A: (tokens, tokens)
                        import math 
                        seeds = []
                        ks=[1,2,3,4,5]
                        dims = (int(math.sqrt(feats.shape[1])), int(math.sqrt(feats.shape[1])))
                        ...
                        if not seeds:
                            continue 
                        print(seeds)
                        avg_attn = torch.sum(A[seeds, :], dim=0)
                        features = (patch_feats.cpu().detach() * avg_attn.unsqueeze(-1)).sum(dim=1)  # (1, D)
                    else:
