...
from .misc import get_object_from_string, is_seq_of
...
class Registry:
    ...
    @property
    def keys(self):
        return [key for key in self._module_dict.keys()]

    @property
    def lower_keys(self):
        return [key.lower() for key in self._module_dict.keys()]
    ...
    def get(self, key: str, case_sensitive=False) -> Optional[Type]:
        """
        ...
        if not isinstance(key, str):
            raise TypeError('The key argument of `Registry.get` must be a str, '
                            f'got {type(key)}')
        ...
        if scope is None or scope == self._scope:
            if case_sensitive and real_key in self._module_dict:
                obj_cls = self._module_dict[real_key]
            if not case_sensitive and real_key.lower() in self.lower_keys:
                for module_dict_key in self.keys:
                    if module_dict_key.lower() == real_key:
                        obj_cls = self._module_dict[module_dict_key]
            elif scope is None:
    ...
    def register(self,
        ...
        if not (name is None or isinstance(name, str) or is_seq_of(name, str)):
            raise TypeError('name must be None, an instance of str, or a sequence of str, '
                            f'but got {type(name)}')
