...
class TrainRunner(BaseTrainRunner, Callbacks):
    ...
    def run(self):
        ...
        if self.args['augment']['backend'].lower() != "pil" and not self.args['augment']['use_v2']:
            # TODO: Support tensor backend in V1?
            raise ValueError("Use --use-v2 if you want to use the tv_tensor or tensor backend.")
        if self.args['augment']['use_v2'] and self.args['dataset']['type'] != "coco":
        ...
        model, model_without_ddp, params_to_optimize = get_model(self.args, num_classes, device)
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs):
            ...
            train_metric_logger = train_one_epoch(model, criterion, optimizer, data_loader, lr_scheduler, device, epoch, self.args.print_freq, scaler)
