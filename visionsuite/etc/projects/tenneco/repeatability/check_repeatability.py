                ...
                for conf_threshold in conf_thresholds:
                    ...
                    for json_file1 in tqdm(json_files1, desc=f"{dir_name}: IoU({iou_threshold}) & Area({area_threshold} & Conf({conf_threshold}))"):
                        ...
                        if vis and is_different:
                            if filename_postfix == "*":
                                imgs1 = glob(osp.join(base_dir, dir_name, f'{case}/exp/vis/{filename}{filename_postfix}.png'))
                                imgs2 = glob(osp.join(base_dir, dir_name, f'{case}/exp2/vis/{filename}{filename_postfix}.png'))
                                imgs3 = glob(osp.join(base_dir, dir_name, f'{case}/exp3/vis/{filename}{filename_postfix}.png'))
                                assert len(imgs1) == 1 and len(imgs1) != 0
                                assert len(imgs2) == 1 and len(imgs2) != 0
                                assert len(imgs3) == 1 and len(imgs3) != 0
                                img1 = cv2.imread(imgs1[0])
                                img2 = cv2.imread(imgs2[0])
                                img3 = cv2.imread(imgs3[0])
                            else:
