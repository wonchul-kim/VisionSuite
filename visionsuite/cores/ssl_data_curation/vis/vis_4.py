...
ax.scatter(ori_data[:, 0], ori_data[:, 1], alpha=0.2)
plt.savefig('/HDD/etc/curation/ori_data.png')
plt.close() 

data = np.load('/HDD/etc/curation/data/level1/centroids.npy', mmap_mode="r")
print("data: ", data.shape)
figh, figw = 1, 1
fig, ax = plt.subplots(figh, figw, figsize=(6 * figw, 5))
ax.scatter(data[:, 0], data[:, 1], alpha=0.2)
plt.savefig('/HDD/etc/curation/data.png')
plt.close()
