...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", type=str, default="/HDD/etc/outputs/most", help="Output directory to store predictions and visualizations."
    )
    ...
    for im_id, inp in enumerate(pbar):
        ...
        with torch.no_grad():
            ...
            if "vit" in args.arch or "ibot" in args.arch:
                ...
                k = k.transpose(1, 2).reshape(nb_im, nb_tokens, -1) # (bs, nh, tokens, head_dim) -> (bs, tokens, nh, head_dim) -> (bs, tokens, all-head_dim)
                q = q.transpose(1, 2).reshape(nb_im, nb_tokens, -1) # (bs, nh, tokens, head_dim) -> (bs, tokens, nh, head_dim) -> (bs, tokens, all-head_dim)
                v = v.transpose(1, 2).reshape(nb_im, nb_tokens, -1) # (bs, nh, tokens, head_dim) -> (bs, tokens, nh, head_dim) -> (bs, tokens, all-head_dim)
