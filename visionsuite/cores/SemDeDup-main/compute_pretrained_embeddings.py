...
def process_images(image_directory, model_dict, batch_size, output_dir, device):
    
    output_dir = osp.join(output_dir, f"{model_dict['model_name']}_{model_dict['ckpt']}")
...
def generate_embeddings(all_image_ids, images_to_paths, model, processor, device, batch_size, regenerate_embeddings, embeddings_file):
    ...
    damaged_image_ids, all_embeddings, all_paths = set(), [], []
