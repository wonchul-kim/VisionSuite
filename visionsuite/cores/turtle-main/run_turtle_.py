...
def _parse_args(args):
    ...
    parser.add_argument('--M', type=int, default=100, help='Number of inner steps at each outer iteration')
    # others
...
def run(args=None):
    ...
    Zs_train = [np.load(ROOT / f"{args.root_dir}/representations/{phi}/{args.dataset}_train.npy").astype(np.float32) for phi in args.phis]
    ...
    task_path = f"turtle_{phis}_innerlr{args.inner_lr}_outerlr{args.outer_lr}_T{args.T}_M{args.M}_{inner_start}_gamma{args.gamma}_bs{args.batch_size}_seed{args.seed}_cluster{C}" 
